...
export function AdminPage() {
  ...
  if (!token) {
    return (
      <div className="mx-auto max-w-7xl px-4 py-16 text-center">
        <h1 className="text-2xl font-bold text-foreground">Admin Login Required</h1>
        <p className="mt-2 text-sm text-foreground/50">Please sign in with an admin account.</p>
        <a href="/account/auth/login?redirect=/admin" className="btn-primary mt-4 inline-block px-6 py-2">Sign In</a>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      {tab === "products" && <AdminProductsTab token={token} />}
      {tab === "orders" && <AdminOrdersTab token={token} />}
      {tab === "inventory" && <AdminInventoryTab token={token} />}
      {tab === "discounts" && <AdminDiscountsTab token={token} />}
      {tab === "metrics" && <AdminMetricsTab token={token} />}
    </div>
  );
}
...
function AdminProductsTab({ token }: { token: string }) {
  ...
  return (
    <div>
      ...
      {showForm && <ProductForm token={token} onSave={handleCreate} />}
...
function AdminOrdersTab({ token }: { token: string }) {
  ...
  async function updateStatus(orderId: string, status: string) {
    ...
    const res2 = await fetch("/api/admin/orders", { headers });
            ...
            {orders.map((o: any) => (
              <tr key={o.id} className="border-b border-primary/5 last:border-0">
                ...
                <td className="p-3 flex gap-2">
                  ...
                  {o.status === "PROCESSING" && <button onClick={() => updateStatus(o.id, "SHIPPED")} className="text-xs text-blue-600 hover:underline">Ship</button>}
...
function AdminInventoryTab({ token }: { token: string }) {
  const [products, setProducts] = useState<any[]>([]);
...
function AdminDiscountsTab({ token }: { token: string }) {
  ...
  async function createCode(form: any) {
    setError("");
    const res = await fetch("/api/admin/discounts", { method: "POST", headers, body: JSON.stringify(form) });
    if (!res.ok) { const d = await res.json(); setError(d.error?.message || "Failed"); return; }
    setShowForm(false);
    const res2 = await fetch("/api/admin/discounts", { headers });
    const data = await res2.json();
    setCodes(data.data || []);
  }

  async function deactivate(id: string) {
    await fetch(`/api/admin/discounts?id=${id}`, { method: "DELETE", headers });
    const res = await fetch("/api/admin/discounts", { headers });
    const data = await res.json();
    setCodes(data.data || []);
  }
...
function AdminMetricsTab({ token }: { token: string }) {
  const [metrics, setMetrics] = useState<any>(null);
  const [loading, setLoading] = useState(true);

  const headers = { Authorization: `Bearer ${token}`, origin: window.location.origin };
  ...
  async function downloadCSV() {
    const res = await fetch("/api/admin/metrics?format=csv", { headers });
